...
export const CellAction = ({ item }) => {
  ...
  const onDelete = async () => {
    try {
      setLoading(true)
      await axios.delete(`/api/${params.storeId}/categories/${item.id}`)
      router.refresh()
    ...
    <>
      ...
      " onClick={() => setOpen(prev => !prev)}>...</button>
        {open && (
          ...
          <div ref={menu} className="absolute top-[100%] left-0 p-2 rounded-md shadow-md bg-white">
            <ul className="flex flex-col gap-2 ">
              ...
              <li className="flex items-center gap-1 cursor-pointer"
                onClick={() => router.push(`/${params.storeId}/categories/${item.id}`)}>
